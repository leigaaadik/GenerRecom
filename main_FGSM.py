...
def get_args():
    ...
    parser.add_argument('--adv_epsilon', default=0.01, type=float, help="Epsilon for FGSM adversarial training on embeddings. 0 to disable.")
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Evaluating Metrics"):
            ...
            with torch.amp.autocast(device_type=device):
                log_feats = model.log2feats(seq, token_type, seq_feat, seq_ts, model.item_emb.weight)
                pos_embs = model.feat2emb(pos, pos_feat, model.item_emb.weight, include_user=False)
                neg_embs = model.feat2emb(neg, neg_feat, model.item_emb.weight, include_user=False)
        ...
        for step, batch in tqdm(enumerate(train_loader), total=num_train_steps_per_epoch, desc=f"Training Epoch {epoch}"):
            ...
            if not torch.isnan(loss) and loss.item() > 0:
                ...
                if args.adv_epsilon > 0:
                    ...
                    if item_emb_grad is not None:
                        # FGSM perturbation
                        delta = args.adv_epsilon * item_emb_grad.sign()
                        perturbed_item_emb = model.item_emb.weight.data + delta
                        ...
                        with torch.amp.autocast(device_type=args.device):
                            adv_loss = model(
                                ...
                                item_embedding_table=perturbed_item_emb
                            )
                        scaler.scale(adv_loss).backward()

                log_json = json.dumps(
                    {'global_step': global_step, 'loss': loss.item(), 'adv_loss': adv_loss.item() if adv_loss is not None else 0, 'epoch': epoch, 'time': time.time()}
                )
                ...
                if adv_loss is not None:
                    writer.add_scalar('Loss/train_adv', adv_loss.item(), global_step)
